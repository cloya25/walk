...
void render(void)
{
    ...
    if(gl.state == STATE_STARTUP){
        ...
        ggprint8b(&r, 16, 0, "G play"); // for the GAMEPLAY
        // i think it works like this
        ggprint8b(&r, 16, 0, message() );
        //ggprint8b(&r, 16, 0, code to call my file);
    }
    ...
    if(gl.state == STATE_PAUSE){
        ...
        glEnable(GL_BLEND);
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        //glColor4f(1.0, 1.0, 0.0, 0.8); //change this one right here
        glColor4f(0, 0, 205, 0.1); //change this one right here
        glTranslated(gl.xres/2 , gl.yres/2, 0);
        glBegin(GL_QUADS);
        glVertex2i(-w,-h);
        glVertex2i(-w, h);
