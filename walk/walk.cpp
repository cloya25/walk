...
class Global {
	...
	GLuint walkTexture;
	Vec box[20];
	unsigned char keys[65535]; //#of values
	Global() {
	    done=0;
	    xres=800;
	    yres=600;
...
void screenCapture(){
    ...
    char ts[64];
    sprintf(ts, "pic%03i.ppm", num++); //very handy for some reason
    FILE* fpo = fopen("screen.ppm", "w");
    if(fpo){
	fprintf(fpo, "P6\n");
	fprintf(fpo, "%i%i\n", gl.xres, gl.yres);
	fprintf(fpo, "255\n");
...
void physics(void)
{
    ...
    // XK_Left ... this is what i'm going to use in this function
...
void render(void)
{
    ...
    glBegin(GL_QUADS);
    if(gl.keys[XK_Right])	{
	glTexCoord2f(tx,      ty+.5); glVertex2i(cx-w, cy-h); // mess with this part here
	glTexCoord2f(tx,      ty); glVertex2i(cx-w, cy+h); // mess with this part here
